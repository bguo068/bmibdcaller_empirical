...
for simulation in simulations:
    ...
    for icaller, caller in enumerate(ibdcallers):
        ...
        if icaller == 0:
            ax.set_title(label)
        IBD.pickle_load(ibdobj_path_res[label, caller]).plot_coverage(
...
def merge_with_meta(df, meta, use_province=False, min_sz=30):
    """merge df with meta"""
    df = df.copy()
    df["Sample"] = df["RealSample"]
    # if use_province is set, use Admin level 1 as population label
    if use_province:
        cols = ["Sample", "Admin level 1"]
        meta1 = meta[cols].copy()
        meta1.columns = ["Sample", "Population"]
    else:
        cols = ["Sample", "Population"]
        meta1 = meta[cols].copy()
    df = df.merge(meta1, on="Sample", how="left")
    sel_pop = (df.Population.value_counts()[lambda s: s >= min_sz]).index.tolist()
    print(df.Population.value_counts())
    print(sel_pop)
    df = df[df.Population.isin(sel_pop)].copy()

    vmax = df.Population.value_counts().max()
    return (df, vmax)


fig, axes = plt.subplots(nrows=2, ncols=6, figsize=(16, 6), constrained_layout=True)
for ilabel, label in enumerate(["structured", "singlepop_AS-SE-E_10_12"]):
    adj_rank_lst = []
    for icaller, caller in enumerate(ibdcallers):
        ifm = ifm_res[(label, caller, "orig")]
        vmax = 0
        if label == "singlepop_AS-SE-E_10_12":
            ifm, vmax = merge_with_meta(ifm, meta, use_province=True)
        else:
            ifm, vmax = merge_with_meta(ifm, meta, use_province=False)
        ...
        ax.imshow(
            df, cmap="Blues", vmin=0, vmax=vmax, interpolation="none", aspect="auto"
        )
